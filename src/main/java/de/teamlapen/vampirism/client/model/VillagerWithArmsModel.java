...
public class VillagerWithArmsModel extends VillagerModel implements ArmedModel {
    ...
    public void setupAnim(@NotNull VillagerRenderState entityIn) {
        ...
        if (entityIn instanceof ILivingEntityRenderStateExtension ext && ext.vampirism$attackTime() > 0.0F) {
            HumanoidArm enumhandside = ext.vampirism$attackArm();
            ModelPart modelrenderer = this.getArmForSide(enumhandside);
            float f1;
            f1 = 1.0F - ext.vampirism$attackTime();
            f1 = f1 * f1;
            f1 = f1 * f1;
            f1 = 1.0F - f1;
            float f2 = Mth.sin(f1 * (float) Math.PI);
            float f3 = Mth.sin(ext.vampirism$attackTime() * (float) Math.PI) * -(this.getHead().xRot - 0.7F) * 0.75F;
