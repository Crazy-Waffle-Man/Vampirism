...
public record LevelingChange(Map<Key<?>, Change<?>> values) {
    ...
    public Holder<? extends IPlayableFaction<?>> getNewFaction() {
        return getNewFaction(VampirismFactions.NEUTRAL);
    }
    ...
    public boolean getNewDracula() {
        return getNewDracula(false);
    }

    public boolean getNewDracula(boolean fallback) {
        DraculaChange factionChange = get(DraculaChange.KEY);
        return factionChange == null ? fallback : factionChange.dracula();
    }
    ...
    public static LevelingChange neutral() {
        return new Builder()
                .faction(VampirismFactions.NEUTRAL)
                .level(0).build();
    }

    public static Builder maxLevel(@NotNull Holder<? extends IPlayableFaction<?>> faction) {
        return new Builder()
                .faction(faction)
                .level(faction.value().getHighestReachableLevel());
    }

    public static Builder maxLord(@NotNull Holder<? extends IPlayableFaction<?>> faction) {
    ...
    public static class Builder {
        ...
        public Builder faction(@NotNull Holder<? extends IPlayableFaction<?>> newFaction) {
            this.changes.put(FactionChange.KEY, new FactionChange(newFaction));
            return this;
        }

        public Builder dracula(boolean isDracula) {
            this.changes.put(DraculaChange.KEY, new DraculaChange(isDracula));
            return this;
        }

        public LevelingChange build() {
            return new LevelingChange(Collections.unmodifiableMap(this.changes));
        }
    }
    ...
    public record LevelChange(int newLevel) implements Change<LevelChange> {
        public static final Key<LevelChange> KEY = new Key<>(VResourceLocation.mod("level"));
    ...
    public record FactionChange(Holder<? extends IPlayableFaction<?>> newFaction) implements Change<FactionChange> {
        public static final Key<FactionChange> KEY = new Key<>(VResourceLocation.mod("faction"));
    ...
    public record LordLevelChange(int newLevel) implements Change<LordLevelChange> {
        public static final Key<LordLevelChange> KEY = new Key<>(VResourceLocation.mod("lord"));
    ...
    public record DraculaChange(boolean dracula) implements Change<DraculaChange> {
        public static final Key<DraculaChange> KEY = new Key<>(VResourceLocation.mod("dracula"));

        @Override
        public Key<DraculaChange> key() {
            return KEY;
        }
    }
