...
public class FactionCommand extends BasicCommand {
    ...
    public static ArgumentBuilder<CommandSourceStack, ?> register(CommandBuildContext buildContext) {
        HolderLookup.RegistryLookup<IFaction<?>> factions = buildContext.lookupOrThrow(VampirismRegistries.Keys.FACTION);
        ...
        for (var listHolder : factions.listElements().toList()) {
            ...
            if (IFaction.is(listHolder, ModFactions.VAMPIRE)) {
                factionCommand.then(Commands.literal("dracula")
    ...
    private static int setDracula(CommandContext<CommandSourceStack> context, @NotNull Collection<ServerPlayer> players) throws CommandSyntaxException {
        for (ServerPlayer player : players) {
            ...
            if (!handler.setFaction(LevelingChange.maxLord(ModFactions.VAMPIRE).dracula(true).build())) {
            ...
            dracula.awardTitle();
            context.getSource().sendSuccess(() -> Component.translatable("command.vampirism.base.dracula.successful", player.getName()), true);
        }
        return 0;
    }
}
